...
class AutoLogin:
	def __init__(self):
		...
		self.url_login = "http://192.168.7.221:801/eportal/?" \
		                 ...
		                 "&queryACIP=0" \
		                 "&loginMethod=1 "
		self.user_passwd_dict = [] # 你以为我会给你账号吗?哈哈哈哈
		self.main()
	...
	def main(self):
		while True:
			while True:
				if breakout or self.check():
					break
				time.sleep(5)
			while True:
				user_and_passwd = random.choice(self.user_passwd_dict)
				if breakout or self.login(user_and_passwd[0], user_and_passwd[1], random.choice(self.user_agent)):
					break
			if stop:
				break
...
class MyTaskBarIcon(wx.adv.TaskBarIcon):
	ICON = "logo.ico"  # 图标地址
	ID_OPEN_AUTO_START = wx.NewId()  # 菜单选项“打开开机自启”的ID
	ID_CLOSE_AUTO_START = wx.NewId()  # 菜单选项“关闭开机自启”的ID
	ID_ABOUT = wx.NewId()  # 菜单选项“关于”的ID
	ID_EXIT = wx.NewId()  # 菜单选项“退出”的ID
	ID_START = wx.NewId()  # 菜单选项“启动”的ID
	ID_STOP = wx.NewId()  # 菜单选项“关闭”的ID
	ID_LINK_OPEN = wx.NewId()  # 启动自动连接
	ID_LINK_CLOSE = wx.NewId()  # 启动自动连接
	TITLE = "校园网登录"  # 鼠标移动到图标上显示的文字
	...
	def __init__(self):
		...
		self.Bind(wx.EVT_MENU, self.on_start, id=self.ID_START)  # 绑定“启动”选项的点击事件
		self.Bind(wx.EVT_MENU, self.on_stop, id=self.ID_STOP)  # 绑定“停止”选项的点击事件
		self.Bind(wx.EVT_MENU, self.on_open_auto_start, id=self.ID_OPEN_AUTO_START)  # 绑定“打开开机自启”选项的点击事件
		self.Bind(wx.EVT_MENU, self.on_clost_auto_start, id=self.ID_CLOSE_AUTO_START)  # 绑定“关闭开机自启”选项的点击事件
		self.Bind(wx.EVT_MENU, self.on_link_open, id=self.ID_LINK_OPEN)  # 绑定“关闭开机自启”选项的点击事件
		self.Bind(wx.EVT_MENU, self.on_link_close, id=self.ID_LINK_CLOSE)  # 绑定“关闭开机自启”选项的点击事件
	...
	# wx.CallLater(1, self.AutoLogin.main())
	...
	# “打开开机自启”选项的事件处理器
	def on_open_auto_start(self, event):
		os.system(
			'reg add "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion'
			'\Run" /v auto_login_xiaoyuanwang /t reg_sz /d "' + os.path.realpath(
				__file__) + '" /f')
		wx.MessageBox('打开开机自启')
	
	# “关闭开机自启”选项的事件处理器
	def on_clost_auto_start(self, event):
		os.system(
			'reg delete "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion'
			'\Run" /v "auto_login_xiaoyuanwang" /f')
		wx.MessageBox('关闭开机自启')
	
	def on_link_open(self, event):
		with open('setting.txt', 'w')as f:
			f.write('1')
		wx.MessageBox('打开启动自动登录')
	
	def on_link_close(self, event):
		with open('setting.txt', 'w')as f:
			f.write('0')
		wx.MessageBox('关闭启动自动登录')
	...
	def getMenuAttrs(self):
		return [
			('启动', self.ID_START),
			('停止', self.ID_STOP),
			('打开开机自启', self.ID_OPEN_AUTO_START),
			('关闭开机自启', self.ID_CLOSE_AUTO_START),
			('打开启动自动登录', self.ID_LINK_OPEN),
			('关闭启动自动登录', self.ID_LINK_CLOSE),
			('关于', self.ID_ABOUT),
			('退出', self.ID_EXIT)
		]
...
if __name__ == "__main__":
	# open_mode = True
	# if os.system(
	#		'reg query "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run" /v "auto_login_xiaoyuanwang"'):
	#	open_mode = False
