...
class AutoLogin:
	def __init__(self):
		...
		self.url_login = 'http://192.168.7.221:801/eportal/?' \
		                 ...
		                 'loginMethod=1 '
		self.numb = 0
		self.user_passwd_dict = [] # 你以为我会给你吗哈哈哈哈
	...
	def solve(self, user_and_passwd):
		global acceptnumb
		oneuseragent = random.choice(self.user_agent)
		self.numb += 1
		if self.login(user_and_passwd[0], user_and_passwd[1], oneuseragent):
			print('失败:' + str(self.numb) + '\n')
			return False
		else:
			print('成功:' + str(self.numb) + '\n')
			return True
	...
	def main(self):
		while True:
			while True:
				...
				time.sleep(5)
			while True:
				if self.solve(random.choice(self.user_passwd_dict)):
					break
